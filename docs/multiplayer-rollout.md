...
# v1.1.1：扫码入口 / QR 候选选择 / 加回模式选择

## 1. QR 候选选择（修复多网卡机器扫码失败）

根因：`/lan` 返回全部非内部 IPv4（PC 上含 TUN/虚拟网卡地址，如 198.18.x/192.168.25.x），
二维码取第一个候选 → 可能是手机不可达的虚拟网卡地址。
修复：候选按常见局域网段（192.168/10/172.16-31）优先排序，TUN/基准段(198.18.x)与
链路本地(169.254.x)垫底；QR 区块展示**可切换的候选 chips**（默认取排序第一）——
多网卡机器上房主可直接点选真实 LAN IP 后重新生成二维码。qrcode 内容不变。

## 2. 扫码加入房间入口（OnlinePage 第三入口）

OnlinePage 新增「扫码加入房间」入口（与联机·双人/三人同视觉体系）→ 进入加入页：
- 顶部指引：加入请用**手机系统相机**扫描房主屏幕上的二维码（应用内相机扫码在
  局域网 http 下不可用——非安全上下文，getUserMedia/BarcodeDetector 均不可用）；
- 表单含**模式选择**（双人/三人，须与房主房间一致）+ 服务器/房间号；
- 加入后按正常流程：等待/二维码（房主侧展示给其它玩家）/开局。

不做桌面摄像头扫码器、不引入扫码 SDK（http 下不可用 + 非必要）。

## 结果

core 86/86 · server 76/76 · web typecheck/build ✅ · 零进程残留。
扫码入口真机流程：PENDING MANUAL。

---

# v1.1.2：扫码入口简化 + 重复点击重新尝试连接

## 1. 扫码入口简化（产品规格）

「扫码加入房间」入口 → **纯说明页**（标题 + 微信/浏览器扫码说明 + 返回），
不再进入重复的加入参数表单（服务器/房间/计时/模式/加入按钮全部移除——
这些与联机·双人/三人设置页重复，且页面标题曾错误写死为三人）。
理由：微信/浏览器扫码 + URL 参数自动加入已正常工作，
应用内无需自建扫码器（局域网 http 下相机扫码不可用）。

## 2. 重复点击重新尝试连接（joinSeq）

根因：useOnlineGame 连接 effect 的依赖是 url/roomId 原始值——
相同参数的重复点击不会触发重新连接（点击无反应的一类来源）。
修复：OnlineConnection 增加可选 `seq`，App 的加入入口每次点击递增
joinSeqRef 并传入 → effect 依赖含 seq → 每次点击都是全新的连接尝试。
NULL_CONNECTION.seq = 0（空闲），退出后归零，不影响既有流程。
